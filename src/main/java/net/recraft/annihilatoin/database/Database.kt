...
object Database {
    ...
    fun getPlayerStatus(rawUUID: UUID): PlayerStats {
    ...
    fun incCount(column: AnnihilationStatsColumn, rawUUID: UUID) {
        val uuid = StringBuilder()
        rawUUID.toString().split("-").forEach { uuid.append(it) }
    ...
    fun addCount(column: AnnihilationStatsColumn, rawUUID: UUID, addCount: Int) {
        val uuid = StringBuilder()
        rawUUID.toString().split("-").forEach { uuid.append(it) }
